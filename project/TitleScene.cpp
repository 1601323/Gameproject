...
void TitleScene::Draw()
{
	...
	if (selectFlag == true) {
		...
		if (_menu == GAME_START) {
			dirMoveCnt += 2;
			DrawGraph(70 - abs(30 - ( 200 + (dirMoveCnt / 2 % 60)) % 59), 350, im.ImageIdReturn("仮image/UI/dirset1.png", SCENE_SELECT), true);
		}
		else if (_menu == GAME_EXPLAIN) {
			dirMoveCnt += 2;
			DrawGraph(70 - abs(30 - (200 + (dirMoveCnt / 2 % 60)) % 59), 500, im.ImageIdReturn("仮image/UI/dirset1.png", SCENE_SELECT), true);
		}
	}
	if (operateFlag == true) {
