...
void TitleScene::Draw()
{
	...
	if (selectFlag == true) {
		...
		if (_menu == GAME_START) {
			dirMoveCnt += 2;
			DrawGraph(170 - abs(30 - ( 200 + (dirMoveCnt / 2 % 60)) % 59), 450, im.ImageIdReturn("仮image/yazirushi.png", SCENE_SELECT), true);
		}
		else if (_menu == GAME_EXPLAIN) {
			dirMoveCnt += 2;
			DrawGraph(170 - abs(30 - (200 + (dirMoveCnt / 2 % 60)) % 59), 530, im.ImageIdReturn("仮image/yazirushi.png", SCENE_SELECT), true);
		}
	}
	if (operateFlag == true) {
